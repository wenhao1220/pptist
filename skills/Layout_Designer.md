...
6. **Data class indicator:** pass through the upstream `dataClass` field on any `chart`/`table` content object unchanged. `"real"` means source-backed values and needs no caption. `"pending"` means the user requested this visual but no verified values were available: preserve its full visual zone and add a readable, unframed note directly below it explaining exactly what source or values are needed. A pending visual must contain no numeric values or table rows. Never output `"scenario"` or illustrative/fabricated data.
...
12. Any `chart`/`table` with `dataClass: "pending"` must get a readable note identifying the missing source/values; `dataClass: "real"` needs no caption. Always pass `dataClass` through unchanged on the content object either way; never create scenario data.
...
- Use the fixed DataEco template recipes: `dataeco-cover` uses `coverArc`; `dataeco-toc`, `dataeco-content`, `dataeco-chart`, `dataeco-table`, `dataeco-kpi`, `dataeco-process`, `dataeco-timeline`, `dataeco-pyramid`, `dataeco-alternating-steps`, `dataeco-orbit-image`, `dataeco-project-hub`, `dataeco-milestone-bar`, `dataeco-why-how-what`, and `dataeco-image-split` use `contentRail`; `dataeco-closing` uses `closingArc`.
- For `dataeco-toc`, make one clear numbered list and no chart/card grid. For `dataeco-chart`, reserve a large chart zone and a separate 2–4 bullet insight zone. For `dataeco-table`, reserve a full-width readable table zone. For `dataeco-why-how-what`, output exactly one title plus a three-item bullets element ordered WHY, HOW, WHAT; each item is already a concise, complete slide summary—never an excerpt ending in `...`. The renderer owns the fixed concentric-circle composition and must not receive extra decorative shapes.
