<template>
  <div class="base-view" :class="{ 'laser-pen': laserPen }">
    ...
    >
      <div class="content">
        <div class="tool-btn page-number" @click="slideThumbnailModelVisible = true">投影片 {{slideIndex + 1}} / {{slides.length}}</div>
        <i-icon-park-outline:write class="tool-btn" v-tooltip="'畫筆工具'" @click="writingBoardToolVisible = true" />
        <i-icon-park-outline:magic class="tool-btn" v-tooltip="'雷射筆'" :class="{ 'active': laserPen }" @click="laserPen = !laserPen" />
        <i-icon-park-outline:stopwatch-start class="tool-btn" v-tooltip="'計時器'" :class="{ 'active': timerlVisible }" @click="timerlVisible = !timerlVisible" />
        <i-icon-park-outline:list-view class="tool-btn" v-tooltip="'演講者檢視'" @click="changeViewMode('presenter')" />
        <i-icon-park-outline:peoples-two class="tool-btn" v-tooltip="'觀眾檢視'" @click="openAudienceView()" />
        <i-icon-park-outline:off-screen-one class="tool-btn" v-tooltip="'退出全屏'" v-if="fullscreenState" @click="manualExitFullscreen()" />
        <i-icon-park-outline:full-screen-one class="tool-btn" v-tooltip="'進入全屏'" v-else @click="enterFullscreen()" />
        <i-icon-park-outline:power class="tool-btn" v-tooltip="'結束放映'" @click="exitScreening()" />
      </div>
    </div>
...
const contextmenus = (): ContextmenuItem[] => {
  return [
    ...
    { divider: true },
    {
      text: autoPlayTimer.value ? '取消自動放映' : '自動放映',
