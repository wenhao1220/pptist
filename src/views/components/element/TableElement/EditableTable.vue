...
// 當前啟用的單元格：當且僅當只有一個選中單元格時，該單元格為啟用的單元格
...
// 將焦點移動到下一個單元格
// 當前行右邊有單元格時，焦點右移
// 當前行右邊無單元格（已處在行末），且存在下一行時，焦點移動至下一行行首
// 當前行右邊無單元格（已處在行末），且不存在下一行（已處在最後一行）時，建立一行並將焦點移動至下一行行首
const tabActiveCell = () => {
  ...
  return [
    {
      text: '插入列',
      children: [
        { text: '到左側', handler: () => insertCol(colIndex) },
        { text: '到右側', handler: () => insertCol(colIndex + 1) },
      ],
    },
    {
      text: '插入行',
