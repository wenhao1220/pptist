<template>
  ...
  >
    <div class="add-slide">
      <div class="btn" @click="createSlide()"><i-icon-park-outline:plus class="icon" />新增投影片</div>
        ...
        <div class="thumbnail-container">
          ...
          >
            ...
            <span class="text" v-else>
              <div class="text-content">{{ element?.sectionTag ? (element?.sectionTag?.title || '無標題節') : '預設節' }}</div>
            </span>
          </div>
          <div
    ...
    <div class="page-number">投影片 {{slideIndex + 1}} / {{slides.length}}</div>
  </div>
</template>
...
const handleClickSlideThumbnail = (e: MouseEvent, index: number) => {
  ...
  // 按住Ctrl鍵，點選投影片，再次點選已選中的頁面則取消選中
  // 如果被取消選中的頁面剛好是當前啟用頁面，則需要從其他被選中的頁面中選擇第一個作為當前啟用頁面
...
// 設定縮圖工具欄聚焦狀態（只有聚焦狀態下，該部分的快捷鍵才能生效）
