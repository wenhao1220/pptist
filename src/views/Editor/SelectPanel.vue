<template>
  <MoveablePanel 
    ...
    :width="200" 
    :height="360" 
    :title="`選擇（${activeElementIdList.length}/${currentSlide.elements.length}）`" 
    :left="-270" 
    :top="90"
    @close="close()"
  >
    <div class="handler" v-if="elements.length">
      <div class="btns">
        <Button size="small" style="margin-right: 5px;" @click="showAllElements()">全部顯示</Button>
        <Button size="small" @click="hideAllElements()">全部隱藏</Button>
      </div>
    ...
    <div class="empty" v-if="!elements.length">本頁無內容</div>
  </MoveablePanel>
</template>
