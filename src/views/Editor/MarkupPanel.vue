<template>
  ...
  >
    <div class="container">
      ...
      <div class="placeholder" v-else>選中圖片、文字、帶文字的形狀，標記型別</div>
    </div>
  </MoveablePanel>
</template>
...
const slideTypeOptions = ref<{ label: string; value: SlideType | '' }[]>([
  { label: '未標記型別', value: '' },
  { label: '封面頁', value: 'cover' },
  { label: '目錄頁', value: 'contents' },
  { label: '過渡頁', value: 'transition' },
  { label: '內容頁', value: 'content' },
  { label: '結束頁', value: 'end' },
])
...
const textTypeOptions = ref<{ label: string; value: TextType | '' }[]>([
  ...
  { label: '副標題', value: 'subtitle' },
  { label: '正文', value: 'content' },
  { label: '列表專案', value: 'item' },
  { label: '列表項標題', value: 'itemTitle' },
  { label: '註釋', value: 'notes' },
  { label: '頁首', value: 'header' },
  { label: '頁尾', value: 'footer' },
  { label: '節編號', value: 'partNumber' },
  { label: '專案編號', value: 'itemNumber' },
])

const imageTypeOptions = ref<{ label: string; value: ImageType | '' }[]>([
  { label: '未標記型別', value: '' },
  { label: '頁面插圖', value: 'pageFigure' },
  { label: '專案插圖', value: 'itemFigure' },
  { label: '背景圖', value: 'background' },
])
