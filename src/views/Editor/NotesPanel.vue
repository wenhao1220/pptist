      ...
      <div class="notes" ref="notesRef">
        <div class="note" :class="{ 'active': activeNoteId === note.id }" v-for="note in notes" :key="note.id" @click="handleClickNote(note)">
          <div class="header note-header">
            ...
            <div class="btns">
              <div class="btn reply" @click="replyNoteId = note.id">回覆</div>
              <div class="btn delete" @click.stop="deleteNote(note.id)">刪除</div>
            </div>
          </div>
          ...
          <div class="replies" v-if="note.replies?.length">
            <div class="reply-item" v-for="reply in note.replies" :key="reply.id">
              <div class="header reply-header">
                ...
                <div class="btns">
                  <div class="btn delete" @click.stop="deleteReply(note.id, reply.id)">刪除</div>
                </div>
              </div>
          ...
          <div class="note-reply" v-if="replyNoteId === note.id">
            <TextArea :padding="6" v-model:value="replyContent" placeholder="輸入回覆內容" :rows="1" @enter.prevent="createNoteReply()" />
            <div class="reply-btns">
              <Button class="btn" size="small" @click="replyNoteId = ''">取消</Button>
              <Button class="btn" size="small" type="primary" @click="createNoteReply()">回覆</Button>
            </div>
          </div>
        </div>
        <div class="empty" v-if="!notes.length">本頁暫無註解</div>
      </div>
      <div class="send">
        <TextArea 
          ref="textAreaRef"
          ...
          :padding="6"
          :placeholder="`輸入註解（為${handleElementId ? '選中元素' : '當前頁投影片' }）`"
          :rows="2"
          @focus="replyNoteId = ''; activeNoteId = ''"
          @enter.prevent="createNote()"
        />
        <div class="footer">
          <i-icon-park-outline:delete class="btn icon" v-tooltip="'清空本頁註解'" style="flex: 1" @click="clear()" />
          <Button type="primary" class="btn" style="flex: 12" @click="createNote()"><i-icon-park-outline:plus /> 新增註解</Button>
        </div>
      </div>
    </div>
  </MoveablePanel>
