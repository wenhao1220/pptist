...
// 元件渲染時，如果存在元素焦點，需要清除
// 這種情況存在於：有焦點元素的情況下進入了放映模式，再退出時，需要清除原先的焦點（因為可能已經切換了頁面）
onMounted(() => {
...
// 點選畫布的空白區域：清空焦點元素、設定畫布焦點、清除文字選區、清空格式刷狀態
...
const handleDblClick = (e: MouseEvent) => {
  ...
  createTextElement({
    left,
    top,
    width: 200 / canvasScale.value, // 除以 canvasScale 是為了與點選選區建立的形式保持相同的寬度
    height: 0,
  })
}
...
const toggleBubbleMenu = () => {
  ...
  message.success(`元素氣泡選單已${showBubbleMenu.value ? '啟用' : '停用'}`)
}
