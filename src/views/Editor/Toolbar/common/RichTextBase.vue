  ...
  <div class="rich-text-base">
    ...
    <ButtonGroup class="row" passive>
      <Popover trigger="click" style="width: 30%;">
        ...
        <TextColorButton first v-tooltip="'文字顏色'" :color="richTextAttrs.color">
    ...
    <ButtonGroup class="row" passive>
      <Popover trigger="click" v-model:value="AIPopoverVisible" style="width: 25%;">
        <template #content>
          <PopoverMenuItem center @click="execAI('美化改寫')">美化</PopoverMenuItem>
          <PopoverMenuItem center @click="execAI('擴寫豐富')">擴寫</PopoverMenuItem>
          <PopoverMenuItem center @click="execAI('精簡提煉')">精簡</PopoverMenuItem>
        </template>
        <CheckboxButton
          first
          style="width: 100%;"
      ...
      <Popover placement="bottom-end" trigger="click" v-model:value="linkPopoverVisible" style="width: 25%;">
        <template #content>
          <div class="link-popover">
            <Input v-model:value="link" placeholder="請輸入超連結" />
            <div class="btns">
              <Button size="small" :disabled="!richTextAttrs.link" @click="removeLink()" style="margin-right: 5px;">移除</Button>
              <Button size="small" type="primary" @click="updateLink(link)">確認</Button>
            </div>
          </div>
        </template>
        <CheckboxButton
    ...
    <div class="row">
      <ButtonGroup style="flex: 1;" passive>
        <Button first style="flex: 1;" v-tooltip="'減小段落縮排'" @click="emitRichTextCommand('indent', '-1')"><i-icon-park-outline:indent-left /></Button>
        <Popover trigger="click" v-model:value="indentLeftPanelVisible">
          <template #content>
            <PopoverMenuItem center @click="emitRichTextCommand('textIndent', '-1')">減小首行縮排</PopoverMenuItem>
          </template>
          <Button last class="popover-btn"><i-icon-park-outline:down /></Button>
        </Popover>
      </ButtonGroup>
      <div style="width: 10px;"></div>
      <ButtonGroup style="flex: 1;" passive>
        <Button first style="flex: 1;" v-tooltip="'增大段落縮排'" @click="emitRichTextCommand('indent', '+1')"><i-icon-park-outline:indent-right /></Button>
        <Popover trigger="click" v-model:value="indentRightPanelVisible">
          <template #content>
            <PopoverMenuItem center @click="emitRichTextCommand('textIndent', '+1')">增大首行縮排</PopoverMenuItem>
          </template>
          <Button last class="popover-btn"><i-icon-park-outline:down /></Button>
        </Popover>
      </ButtonGroup>
...
const updateLink = (link?: string) => {
  ...
  if (!link || !linkRegExp.test(link)) return message.error('不是正確的網頁連結地址')
...
const execAI = async (command: string) => {
  ...
  if (!content) return message.error('沒有可以執行的文本內容')
  ...
  if (typeof stream === 'object' && stream.state === -1) {
    return message.error('該模型API的併發數過高，請更換其他模型重試')
  }
