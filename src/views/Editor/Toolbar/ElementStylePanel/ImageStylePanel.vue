<template>
  <div class="image-style-panel">
    ...
    <ButtonGroup class="row" passive>
      <Button first style="width: calc(100% - 32px);" @click="clipImage()"><i-icon-park-outline:tailoring /> 裁剪圖片</Button>
      <Popover trigger="click" v-model:value="clipPanelVisible" style="width: 32px;">
        <template #content>
          <div class="clip">
            <div class="title">按形狀：</div>
    ...
    <FileInput @change="files => replaceImage(files)">
      <Button class="full-width-btn"><i-icon-park-outline:transform /> 替換圖片</Button>
    </FileInput>
    <Button class="full-width-btn" @click="resetImage()"><i-icon-park-outline:undo /> 重置樣式</Button>
    <Button class="full-width-btn" @click="setBackgroundImage()"><i-icon-park-outline:theme /> 設為背景</Button>
  </div>
</template>
