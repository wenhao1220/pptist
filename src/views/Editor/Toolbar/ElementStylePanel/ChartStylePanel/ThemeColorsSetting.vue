<template>
  <div class="theme-colors-setting">
    <div class="title">圖表主題配色</div>

    <div class="list">
      <div class="row" v-for="(item, index) in themeColors" :key="index">
        <div class="label" style="width: 40%;">主題配色{{ index + 1 }}：</div>
        <Popover trigger="click" style="width: 60%;">
          ...
          <div class="color-btn-wrap" style="width: 100%;">
            ...
            <div class="delete-color-btn" v-tooltip="'刪除'" @click.stop="deleteThemeColor(index)" v-if="index !== 0"><i-icon-park-outline:close-small /></div>
          </div>
        </Popover>
      </div>
      <Button
        ...
        @click="addThemeColor()"
      >
        <i-icon-park-outline:plus /> 新增主題色
      </Button>
    </div>

    <Button class="btn" type="primary" @click="setThemeColors()">確認</Button>
  </div>
</template>
