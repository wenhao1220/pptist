  ...
  <div class="element-animation-panel">
    <div class="element-animation" v-if="handleElement">
      ...
      >
        ...
        <Button class="element-animation-btn" @click="handleAnimationId = ''">
          <i-icon-park-outline:effects /> 新增動畫
        </Button>
      </Popover>
    </div>

    <div class="tip" v-else><i-icon-park-outline:click style="margin-right: 5px;" /> 選中畫布中的元素新增動畫</div>
      ...
      <template #item="{ element }">
        <div class="sequence-item" :class="[element.type, { 'active': handleElement?.id === element.elId }]" @click="selectElement(element.elId)">
          <div class="sequence-content">
            ...
            <div class="handler">
              <i-icon-park-outline:play-one class="handler-btn" v-tooltip="'預覽'" @click.stop="runAnimation(element.elId, element.effect, element.duration)" />
              <i-icon-park-outline:close-small class="handler-btn" v-tooltip="'刪除'" @click.stop="deleteAnimation(element.id)" />
            </div>
          </div>
          ...
          <div class="configs" v-if="handleElementAnimation[0]?.elId === element.elId">
            ...
            <div class="config-item">
              <div style="width: 35%;">觸發方式：</div>
              <Select
                :value="element.trigger"
                @update:value="value => updateElementAnimationTrigger(element.id, value as AnimationTrigger)"
                style="width: 65%;"
                :options="[
                  { label: '主動觸發', value: 'click' },
                  { label: '與上一動畫同時', value: 'meantime' },
                  { label: '上一動畫之後', value: 'auto' },
                ]"
              />
            </div>
            <div class="config-item">
              <Button style="width: 100%;" @click="openAnimationPool(element.id)"><i-icon-park-outline:switch /> 更換動畫</Button>
            </div>
          </div>
        </div>
      </template>
    ...
    <template v-if="animationSequence.length >= 2">
      ...
      <Button @click="runAllAnimation()">
        <i-icon-park-outline:pause v-if="animateIn" /><i-icon-park-outline:play-one v-else /> {{ animateIn ? '停止預覽' : '預覽全部'}}
      </Button>
    </template>
  </div>
</template>
...
const tabs: TabItem[] = [
  { key: 'in', label: '入場', color: '#68a490' },
  { key: 'out', label: '退場', color: '#d86344' },
  { key: 'attention', label: '強調', color: '#e8b76a' },
]
...
// 動畫選擇面板開啟600ms後再移除遮罩層，否則打開面板後迅速滑入滑鼠預覽會導致抖動
