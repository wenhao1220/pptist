<template>
  <div class="theme-styles-extract">
        ...
        <div class="values">
          <div class="value-wrap" v-for="(item, index) in themeStyles.fontNames" :key="item">
            ...
            <div class="handler">
              ...
              <div class="config-btn" @click="selectedIndex.fontName = index">選擇</div>
              <div class="config-btn" @click="updateTheme({ fontName: item }); selectedIndex.fontName = index">應用到主題</div>
            </div>
          </div>
        </div>
      </div>
        ...
        <div class="values">
          <div class="value-wrap" v-for="(item, index) in themeStyles.fontColors" :key="item">
            ...
            <div class="handler">
              ...
              <div class="config-btn" @click="selectedIndex.fontColor = index">選擇</div>
              <div class="config-btn" @click="updateTheme({ fontColor: item }); selectedIndex.fontColor = index">應用到主題</div>
            </div>
          </div>
        </div>
      </div>
        ...
        <div class="values">
          <div class="value-wrap" v-for="(item, index) in themeStyles.backgroundColors" :key="item">
            ...
            <div class="handler">
              ...
              <div class="config-btn" @click="selectedIndex.backgroundColor = index">選擇</div>
              <div class="config-btn" @click="updateTheme({ backgroundColor: item }); selectedIndex.backgroundColor = index">應用到主題</div>
            </div>
          </div>
        </div>
      </div>
      <div class="config-item" v-if="themeStyles.themeColors.length">
        <div class="label">主題色：<span class="tip">（點選色塊排除不要的顏色）</span></div>
    ...
    <div class="btns">
      <Button class="btn" type="primary" @click="updateAllThemes()"><i-icon-park-outline:check /> 將選中配置儲存為主題</Button>
    </div>
  </div>
</template>
...
const tabs: TabItem[] = [
  { key: 'single', label: '從當前頁中提取' },
  { key: 'all', label: '從全部投影片提取' },
]
...
const updateAllThemes = () => {
  ...
  if (themeColors.length > 6) {
    ...
    message.warning('主題色超出數量限制，已自動選取前6個')
  }
