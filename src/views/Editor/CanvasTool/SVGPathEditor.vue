<template>
  <div class="svg-path-editor">
    <div class="container">
      ...
      <div class="svg-panel">
        <div class="panel-section">
          ...
          >
            <RadioButton value="L" :disabled="activePointIndex === 0">直線</RadioButton>
            <RadioButton value="Q" :disabled="activePointIndex === 0">曲線Q</RadioButton>
            <RadioButton value="C" :disabled="activePointIndex === 0">曲線C</RadioButton>
            <RadioButton value="A" :disabled="activePointIndex === 0">圓弧</RadioButton>
          </RadioGroup>
        </div>
        ...
        <template v-if="activePoint.q">
          <Divider :margin="20" />
          <div class="panel-section">
            <div class="section-title">控制點：</div>
            <div class="input-row">
              <NumberInput
                class="number-input"
                :min="CANVAS_MIN"
        ...
        <template v-if="activePoint.a">
          <Divider :margin="20" />
          <div class="panel-section">
            <div class="section-title">圓弧：</div>
            <div class="input-row">
              <NumberInput
                class="number-input"
                :min="0"
            ...
            <div class="input-row">
              ...
              >
                <template #prefix>旋轉：</template>
              </NumberInput>
            </div>
            <div class="checkbox-row">
              <Checkbox :value="activePoint.a.laf === 1" @update:value="value => updateArcParam('laf', value ? 1 : 0)">大弧</Checkbox>
              <Checkbox :value="activePoint.a.sf === 1" @update:value="value => updateArcParam('sf', value ? 1 : 0)">順時針</Checkbox>
            </div>
          </div>
        </template>
        ...
        <div class="panel-section">
          <Checkbox :value="closePath" @update:value="value => closePath = value">閉合路徑</Checkbox>
        </div>
    ...
    <div class="footer">
      <div class="tooltips">Tips: 雙擊追加直線，右鍵追加線段或刪除點；拖動端點和控制點調整路徑。</div>
      <div class="footer-actions">
        <Button @click="emit('close')">關閉</Button>
        <Button type="primary" :disabled="!canInsert" @click="insert()">確認</Button>
      </div>
    </div>
  </div>
</template>
