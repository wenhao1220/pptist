<template>
  <div class="media-input">
    ...
    <template v-if="type === 'video'">
      <Input v-model:value="videoSrc" placeholder="請輸入影片地址，e.g. https://xxx.mp4"></Input>
      <div class="btns">
        <FileInput accept="video/*" @change="files => uploadVideo(files)">
          <Button><i-icon-park-outline:upload /> 上傳本地影片</Button>
        </FileInput>
        <div class="group">
          <Button @click="emit('close')" style="margin-right: 10px;">取消</Button>
          <Button type="primary" @click="insertVideo()">確認</Button>
        </div>
      </div>
    </template>

    <template v-if="type === 'audio'">
      <Input v-model:value="audioSrc" placeholder="請輸入音訊地址，e.g. https://xxx.mp3"></Input>
      <div class="btns">
        <FileInput accept="audio/*" @change="files => uploadAudio(files)">
          <Button><i-icon-park-outline:upload /> 上傳本地音訊</Button>
        </FileInput>
        <div class="group">
          <Button @click="emit('close')" style="margin-right: 10px;">取消</Button>
          <Button type="primary" @click="insertAudio()">確認</Button>
        </div>
      </div>
    </template>
  </div>
...
const insertVideo = () => {
  if (!videoSrc.value) return message.error('請先輸入正確的影片地址')
  emit('insertVideo', { src: videoSrc.value })
}

const insertAudio = () => {
  if (!audioSrc.value) return message.error('請先輸入正確的音訊地址')
