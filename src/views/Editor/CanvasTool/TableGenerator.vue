<template>
  <div class="table-generator">
    <div class="title">
      ...
      <div class="right" @click="isCustom = !isCustom">{{ isCustom ? '返回' : '自定義'}}</div>
    </div>
    <table 
      @mouseleave="endCell = []" 
    ...
    <div class="custom" v-else>
      <div class="row">
        <div class="label" style="width: 25%;">行數：</div>
        <NumberInput
          :min="1"
          :max="20"
      ...
      <div class="row">
        <div class="label" style="width: 25%;">列數：</div>
        <NumberInput
          :min="1"
          :max="20"
      ...
      <div class="btns">
        ...
        <Button class="btn" type="primary" @click="insertCustomTable()">確認</Button>
      </div>
    </div>
  </div>
</template>
...
const insertCustomTable = () => {
  if (customRow.value < 1 || customRow.value > 20) return message.warning('行數/列數必須在0~20之間！')
  if (customCol.value < 1 || customCol.value > 20) return message.warning('行數/列數必須在0~20之間！')
