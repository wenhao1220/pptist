    ...
    <div class="btns">
      <div class="left">
        ...
        <Popover trigger="click" placement="top" v-model:value="chartTypeSelectVisible">
          ...
          <span class="change">點選更換</span>
        </Popover>
      </div>
      <div class="right">
        <Button class="btn" @click="closeEditor()">取消</Button>
        <Button class="btn" @click="clear()">清空資料</Button>
        <Button type="primary" class="btn" @click="getTableData()">確認</Button>
      </div>
    </div>
  </div>
</template>
...
const getTableData = () => {
  ...
  // 第一行為系列名，第一列為專案名，實際資料從第二行第二列開始
  for (let rowIndex = 1; rowIndex < row; rowIndex++) {
    let labelsItem = `類別${rowIndex}`
...
const changeSelectRange = (e: MouseEvent) => {
  ...
  document.onmouseup = e => {
    ...
    // 拖拽結束時，範圍超過格子一半自動擴大到下一格（如拖動到一格半多的位置，會自動擴充套件到兩格，橫豎都同理）
