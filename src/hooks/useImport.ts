...
export default () => {
  ...
  const calculateRotatedPosition = (
    ...
    aw: number, // A 的寬
    ah: number, // A 的高
    bx: number, // B 相對 A 的 x (ox)
    by: number, // B 相對 A 的 y (oy)
    bw: number, // B 的寬
    bh: number, // B 的高
    ak: number, // A 的旋轉角度（度，正順時針）
    bk: number, // B 的旋轉角度（度，正順時針）
  ) => {
  ...
  // 匯入PPTX檔案
  const importPPTXFile = (files: FileList | File[], options?: { cover?: boolean; fixedViewport?: boolean; asTemplate?: boolean }): Promise<void> => {
    return new Promise((resolve, reject) => {
      ...
      const { cover, fixedViewport, asTemplate } = { ...defaultOptions, ...options }
      ...
      for (const item of json.slides) {
          ...
          for (const el of sortedElements) {
            ...
            if (el.type === 'text') {
              ...
              if (el.chartType === 'scatterChart' || el.chartType === 'bubbleChart') {
                labels = el.data[0].map((item, index) => `座標${index + 1}`)
        ...
        // Phase 2: 前端動態補償渲染 (讀取 Slide Notes 裡隱寫的 Chart 資料)
        if (slide.remark) {
          const regex = /\[PPTIST_CHART_DATA_START\]([\s\S]*?)\[PPTIST_CHART_DATA_END\]/g;
          let match;
          while ((match = regex.exec(slide.remark)) !== null) {
            try {
              // 1. 移除 HTML 標籤
              let jsonStr = match[1].replace(/<[^>]*>?/gm, '').trim();
              
              // 2. 致命關鍵：還原 XML/HTML 轉義的雙引號與符號
              ...
              // 3. 強制補上 nanoid 與確保 type 為 chart
              ...
              // 4. 安全無縫地推入畫布陣列
              slide.elements.push(newChartElement as any);
            } catch (e) {
              console.error('Failed to parse chart data from notes. 原字串為:', match[1], e);
            }
          }
          // 清理 remark，不要把隱寫資料暴露給使用者
          slide.remark = slide.remark.replace(/\[PPTIST_CHART_DATA_START\][\s\S]*?\[PPTIST_CHART_DATA_END\]/g, '').trim();
          // 若清理後只剩空白或無意義的空標籤，則設為空
          if (slide.remark.replace(/<[^>]*>?/gm, '').trim() === '') {
            slide.remark = '';
          }
        }
      ...
      if (asTemplate) {
        slidesStore.setTemplateSlides(slides)
      }
      else if (cover) {
        slidesStore.updateSlideIndex(0)
