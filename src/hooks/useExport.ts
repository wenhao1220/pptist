...
export default () => {
  ...
  // 格式化顏色值為 透明度 + HexString，供pptxgenjs使用
  ...
  // 將HTML字串格式化為pptxgenjs所需的格式
  // 核心思路：將HTML字串按樣式分片平鋪，每個片段需要繼承祖先元素的樣式資訊，遇到塊級元素需要換行
          ...
          for (let i = 0; i < el.data.length; i++) {
            ...
            for (let j = 0; j < row.length; j++) {
              ...
              const cellOptions: pptxgen.TableCellProps = {
                ...
                valign: 'middle',
                fontFace: cell.style?.fontname || '微軟雅黑',
